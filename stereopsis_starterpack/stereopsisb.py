...
win.set_fullscreen(fullscreen = False , screen = screens[use_scr]) # Present secondary display
...
def load():
    global i, right_half, left_half, ku, kd, kud, list_a, list_b, da
    tc = 0 #Count transients
...
def scheduling():
    # Scheduling flow
    clk.schedule_once(delete, 10.0)
    clk.schedule_once(get_results, 11.0)
    clk.schedule_once(exit_routine, 11.0)
    
def get_results(dt):
    global ku, kud, cdt, tcs, mdt, dtstd, kud_list
